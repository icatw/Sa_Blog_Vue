<template>
  <el-card class="main-card">
    ...
    <el-dialog :visible.sync="isExport" width="30%">
      ...
      <div>
        <div :style="{ fontSize: '1rem' }">是否导出选中文章为
          <span :style="{ color: 'red', fontWeight: 'bold', paddingLeft: '0.2rem', paddingRight: '0.2rem' }">{{
              exportType
            }}</span>文件？
        </div>
      </div>
      <div slot="footer">
        <el-button @click="isExport = false;exportType=null">取 消</el-button>
...
export default {
  ...
  methods: {
    ...
    async download(url) {
      ...
      const n_url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = n_url;
      // 获取url中的文件名(最后一个/之后的字符)
      const fileName = url.substring(url.lastIndexOf('/') + 1);
      link.setAttribute('download', fileName);
      // link.setAttribute('download', 'file.md');
